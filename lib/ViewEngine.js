...
class ViewEngine {
  ...
  constructor(bucket, initUrl) {
    ...
    this.fetchUrls = this.njkLoader.fetchUrls;
  ...
  async handleSeo(html, opts, locals) {
    if (opts.template.parseSeo) {
      const engine = consolidate['nunjucks'];
      const title = await engine.render(SEO_HTML, locals);
  ...
  async render(opt) {
    ...
    opt.template = opt.template || { engine: 'nunjucks' };
    ...
    if (urls.length > 0) {
      const rst = this.fetchUrls(urls);
      rst.forEach((v) => {
        const k = widgetUrls[v.url];
        if (k) {
          const content = fs.readFileSync(v.path, 'utf8');
          if (v.type.toLowerCase() == 'application/json') {
    ...
    html = await this.handleSeo(html, opt, locals);
