...
class ViewEngine {
  ...
  async render(opt = {}) {
    ...
    let engineOptions = opt.template.engineOptions;
    let state = Object.assign({}, opt.state);
    let html  = '';
    ...
    if (opt.extra) {
      if (opt.extra.headAppends) {
        debug('perform extra head prepends');
        html = html.replace('</head>', opt.extra.headAppends + '</head>');
      }

      if (opt.extra.contentPrepends) {
        debug('perform extra content prepends');
        if (html.indexOf('<body') > -1) {
          const ptrn = /\<body\.*\>/mg;
          const match = ptrn.exec(html);
          html = html.replace(match, match + opt.extra.contentPrepends);
        } else {
          html = opt.extra.contentPrepends + html;
        }
      }

      if (opt.extra.contentAppends) {
        debug('perform extra content appends');
        if (html.indexOf('</body>') > 0) {
          html = html.replace('</body>',  opt.extra.contentAppends + '</body>');
        } else {
          html = html + opt.extra.contentAppends;
        }
      }
    }
