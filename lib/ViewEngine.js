...
const PWD = __dirname;
const SEO_HTML = fs.readFileSync(path.join(PWD, 'seo.html'), 'utf8');
...
class ViewEngine {
  ...
  async handleSeo(html, seo) {
    if (seo) {
      const engine = consolidate['nunjucks'];
      const title = await engine.render(SEO_HTML, { seo: seo });
      ...
      const ptrn = /<script type="text\/seo">(^<\\script>)*<\/script>/img;
      const match = ptrn.exec(html);
      if (match && match[0] && match[0].length > 32) {
        // remove from body
        html = html.replace(match[0], '');
        html = this.replaceTitle(html, '<title>' + match[0].substring(14).replace('</script>', '</title>'));
      }
    }
  ...
  async render(opt = {}) {
    ...
    if (opt.template.engine === 'nunjucks') {
      ...
      let rst = await engine(absPath, state);
      html = (rst + '').trim()
      debug('my html:', html);
    }
    ...
    html = await this.handleSeo(html, opt.seo);
    ...
    if (opt.template.minify) {
      debug('using `minify` package to beautify HTML');
      html = minifier(html + '');
    }
